...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        cent = find_closest(restaurant_location(restaurant), centroids)
        pairs.append([cent, restaurant])
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    lat = [restaurant_location(rest)[0] for rest in cluster]
    lon = [restaurant_location(rest)[1] for rest in cluster]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    pairs = zip(xs, ys)
    S_xy = sum([(pair[0] - mean(xs)) * (pair[1] - mean(ys)) for pair in pairs])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    #return a predictor with max r_squared in list of [predictor, r_squared] 

    best_pred = ()
    best_r2 = 0
    for feature in feature_fns:
        ret = find_predictor(user, reviewed, feature)
        if ret[1] > best_r2: 
            best_r2 = ret[1]
            best_pred = ret
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            user_rate = user_rating(user, restaurant_name(restaurant))
            rating_dict[name] = user_rate
        else:
